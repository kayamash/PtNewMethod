...
void PtNewMethod::Loop(Int_t ev,std::string name,Int_t proc){
    ...
    bool LUTcheck = LUT.getLUTparameter(pSA_sAddress,m_poff_charge,poff_eta,poff_phi,LUTparameter,phiInteg);
    ...
    if(LUTparameter[4] != -1)m_h_SectorPhiIntegral[LUTparameter[4] - 1]->Fill(phiInteg);
    cout<<LUTparameter[0]<<"   "<<LUTparameter[1]<<"   "<<LUTparameter[2]<<"   "<<LUTparameter[3]<<"   "<<LUTparameter[4]<<"   "<<phiInteg<<endl;
...
void PtNewMethod::Finalize(TFile *tf1,std::string filenameA,std::string filenameB){
	...
	for(Int_t charge = 0; charge < 2;++charge){//positive,negative
		for(Int_t sector = 0; sector < 5;++sector){//Large,LargeSpecial sector11,LargeSpecial sector 15,Small,SmallSpecial
        	...
        	for(Int_t eta = 0; eta < 16;++eta){
        		for(Int_t phi = 0; phi < 15;++phi){
